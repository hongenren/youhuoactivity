...
function get_job_order_list(index,data,type){
    ...
    //     }
    // }
    var times = ChangeHourMinutestr(data.validTime);
    html += "<li>\
        ...
        if (newTime >= createTime) {//场次开始前4小时
            if (newTime <= newDay) {//开始当天24点
                if(un!=undefined && un!=null && un!=''){
                    html += "<a href=\""+encodeURI(data.qrCode)+"/"+un+"\" >打卡</a>";
                }else{
...
function click_jump_msg(url,sectionCode,ticket,mark,type,this1,time){
    ...
    }else if(ticket_msg.data.state == -10){
...
function get_manage_order_list(index,data,type){
    ...
    html += "<li>\
                <div class=\"warp_dd_o_sp\">\
                    ...
                    <div class=\"warp_dd_o_st\" data-arrivetime=\""+arriveTime.date + " " + arriveTime.str+"\">"
    //html += "<a href=\"javascript:void(0)\">讨论</a>";
    html += "<a href=\"../managementact/managementact_content_personal.html?type=70&activityCode="+data.activityCode+"&sectionCode="+data.sectionCode+"\">管理</a>";
    html += "<a href=\"javascript:void(0)\" data-lat=\""+data.lat+"\" data-lng=\""+data.lng+"\" data-address=\""+data.address+"\" data-mobile=\""+data.contactPhone+"\" data-title=\""+data.title+"\" class=\"tc_dt_p\">地图</a>";
    //【使用中】、【已结算】的票，评价按钮全部显示，可用
    if(data.state==20||data.state==30){
...
function get_fieldadmin_order_list(index,data,type){
    ...
    html += "<li>\
                <div class=\"warp_dd_o_sp\">\
                    ...
                    <div class=\"warp_dd_o_st\" data-arrivetime=\""+arriveTime.date + " " + arriveTime.str+"\">";
    //html += "<a href=\"javascript:void(0)\">讨论</a>"
    html += "<a href=\"../managementact/managementact_content_personal.html?type=80&activityCode="+data.activityCode+"&sectionCode="+data.sectionCode+"\">管理</a>";
    html += "<a href=\"javascript:void(0)\" data-lat=\""+data.lat+"\" data-lng=\""+data.lng+"\" data-address=\""+data.address+"\" data-mobile=\""+data.contactPhone+"\" data-title=\""+data.title+"\" class=\"tc_dt_p\">地图</a>";
    //【使用中】、【已结算】的票，评价按钮全部显示，可用
    if(data.state==20||data.state==30){
        html += "<a href=\"./evaluate.html?ticket="+data.ticket+"\">评价</a>";
...
function get_temporaryadmin_order_list(index,data,type){
    ...
    html += "<li>\
                <div class=\"warp_dd_o_sp\">\
                    ...
                    <div class=\"warp_dd_o_st\" data-arrivetime=\""+arriveTime.date + " " + arriveTime.str+"\">";
    //html += "<a href=\"javascript:void(0)\">讨论</a>"
    html += "<a href=\"../managementact/managementact_content_personal.html?type=90&activityCode="+data.activityCode+"&sectionCode="+data.sectionCode+"\">管理</a>";
    html += "<a href=\"javascript:void(0)\" data-lat=\""+data.lat+"\" data-lng=\""+data.lng+"\" data-address=\""+data.address+"\" data-mobile=\""+data.contactPhone+"\" data-title=\""+data.title+"\" class=\"tc_dt_p\">地图</a>";
    //【使用中】、【已结算】的票，评价按钮全部显示，可用
    if(data.state==20||data.state==30){
        html += "<a href=\"./evaluate.html?ticket="+data.ticket+"\">评价</a>";
...
$(".orgTypes").live("click",function(){
    $("#screening").html($(this).html());
    $(".selectList").hide();
    var d = {'pageNumber':1,'pageSize':5,'user':user_msg.userId,'months':user_msg.userId}
    if($(this).attr("data-id")!=""){
        d.months = $(this).attr("data-id");
    }else{
        delete d.months;
    }
    var html ='';
