$(function(){
    $(".warp_cs").click(function(){
    	//循环日期
        var endDate   = $(".leavetime").text();
        var startDate = $(".entertime").text();
        ...
        if(!old_class_val){
	        ...
	        while(chan_times>=0){
	            ...
	            voDate += '<div class="swiper-slide '+m_class+'" style="height: 42px;align-items: center;cursor: pointer;">\
			                        <a href="javascript:;">\
										<span style="line-height: 42px;display: inline;">'+month+"-"+day+'</span>\
									</a>\
			                  </div>';
	            ...
	            voDate_change += '<li class="warp_pws_n_so" '+m_class_change+'>\
							            <div class="warp_pws_n_si">\
                                            ...
                                            <menu class="warp_pws_n_sy1" style="padding-top:0px;">\
                                               <a href="javascript:void(0)" class="copyA" onclick="copySession(\''+month+day+'\');">复制'+month+'月'+day+'日场次设置至其他日期</a>\
                                            </menu>\
							                <menu class="warp_pws_n_sy" style="padding-top:0px;">\
							                    <a href="javascript:void(0)" id="singleSession" name="singleSessionName" class="thisclass">单天单场次</a>\
							                    <a href="javascript:void(0)" id="moreSession" name="moreSessionName">单天多场次</a>\
							                </menu>\
							            </div>\
	...
	$(function(){
	    $(".warp_pws_n_st li .warp_pws_n_sj a").live('click',function(){
            ...
            var len = $(this).parent().parent().parent().find('.cla').length;
	        if(len==2){
    ...
    $("#setClassBtn").live('click',function(){
		...
		$.each(classformVal.day,function(key,val){
            ...
            if(!compareTime(datetime,endtime)){
                layer.open({ content: val+'开始时间必须晚于当前时间!' ,skin: 'msg' ,time: 2 });
                setStatus = 0;
                return false;
            }
            if(classformVal.startTime[key] >= classformVal.endTime[key]){
            ...
            if(key==0){
                dangDay = classformVal.day[key];
            }
			if(copySession_if==1){
                if(classformVal.type[key]==20 || classformVal.type[key]==30 || classformVal.type[key]==40){
                    if(dangDay!=classformVal.day[key]){
                        return false;
                    }
                }else{
                    if(key==1){
                ...
                result[key] = JSON.stringify({"endTime": classformVal.endTime[key], "startTime": classformVal.startTime[key], "type": classformVal.type[key]});
			}else{
        ...
        $(".section_span").html("已设置")
        $(".warp_cp").animate({left:"100%"})
        $(".warp_cc_p").animate({left:"100%"})
    })

})
var copySession_if = 0;
$('.demo_time').live('change',function(){
    copySession_if = 0;
})
function copySession(id_){
    copySession_if = 1;
    if($("#singleSession").is('.thisclass')){
        $("a[name=singleSessionName]").each(function(){
            if(!$(this).is('.thisclass')){
                $(this).addClass("thisclass");
                $(this).next().removeClass("thisclass");
                switch_(this);
            }
        })
    ...
    if($("#moreSession").is('.thisclass')){
        $("a[name=moreSessionName]").each(function(){
        	if(!$(this).is('.thisclass')){
                $(this).addClass("thisclass");
                $(this).prev().removeClass("thisclass");
                switch_(this);
			}
        })
...
function switch_(obj_){
    $(obj_).addClass("thisclass").siblings().removeClass("thisclass");
    $(obj_).parents(".warp_pws_n_si").next("ul").find("li").hide();
    $(obj_).parents(".warp_pws_n_si").next("ul").find("li").eq($(obj_).index()).show()
    var index = $(this).index();
    var num = index==1?2:1; //判断index选取值改变单双场次
    st = $(obj_).parent().parent().next('.warp_pws_n_st');
    st.find('li').hide()
    st.find('input').attr("disabled",true);//不让提交
    var a = st.find('.t_type').val(num);
    st.find('li').eq($(obj_).index()).show();
    st.find('li').eq($(obj_).index()).find('input').attr("disabled",false);//让提交
}
