    ...
    var calendarSwitch = (function() {
        ...
        calendarSwitch.prototype = { /*说明：初始化插件*/
            ...
            init: function() {
                ...
                var html = "<div class='headerWrapper'><div class='headerTip'>请选择活动日期</div><div class='comfire'>完成</div></div><table class='dateZone'><tr><td class='colo'>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td class='colo'>六</td></tr></table>" + "<div class='tbody'></div>"
                ...
                $(me.comfire).on('click', function(event) {
                    event.preventDefault();
                    var old_startDate = $('#startDate').val();  //开始时间的天数
                    var old_endDate = $('#endDate').val();      //结束时间的天数
                        ...
                        if ($(this).text() == '结束') {
                            ...
                            $(me.sections).find('.tbody .rz').each(function(index, element) {
                                ...
                                if ($(this).text() == '结束') {
                                    ...
                                    var day = $(this).parent().text().split('结束')[0];
                                    if(day < 10){
                                        day = "0" + day;
                                    }
                            ...
                            //如果开始没值
                            if (st) {
                                //如果开始时间和结束时间全等于上一次选择时间则不触发场次修改
                                if(st != old_startDate || en != old_endDate){
                                    $(".warp_cs").children('span').html("请选择")
                                    $("#carousel").empty();
                ...
                $(arry1).on('click', function(index) {
                    ...
                    if (flag == 0) {
                        ...
                        $(me.sections).find('.rz').each(function(index, element) {
                            if ($(this).text() == '开始') {
                                $(this).parent('td').append('<span class="hover ruzhu_hover">选择开始日期</span>');
