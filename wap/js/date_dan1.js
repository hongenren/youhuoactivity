...
var date = function ($) {
  ...
  $.fn.hotelDate = function (options) {
    ...
    function initTemplate() {
      ...
      // 主容器模板
      var dateTemplate = '\n        <div class =\'date container c-gray\' style=\'position:absolute;\'>\n          <h4 class="tac bold">请选择活动发布\u65F6\u95F4</h4>\n          <div class=\'close-btn\'>\u786E\u5B9A</div>\n        </div>      \n      ';
      ...
      $('.action').each(function (index, item) {
        ...
        for (var i = 0; i < days + nowweek; i++) {
          ...
          } else if (i > nowdate.getDate() + nowweek - 2 && dateArr[index].getMonth() === nowdate.getMonth()) {
            // 当月已经过去的日期 不能点击
              listIndex++;
      ...
      $('.close-btn').on('click', function () {
        ...
        var date = new Date((enterTime.replace(/\-/g,'/'))+" 00:00:00");
        var newDate =  new Date(nowYear, nowMonth, nowDay);
        if(date>newDate){
            layer.open({
                content: '补登未发活动时只能选择当天以前的日期!',
                skin: 'msg',
                time: 2
            });
          return false;
        ...
        $('.entertime').text(enterTime);
        $('.leavetime').text(enterTime);
        $('.input-enter').val(enterTime);
        $('.input-leave').val(enterTime);
      });
    ...
    function getDate(num) {
      ...
      // console.log(new Date(dateArr))z
    }
    ...
    function initDay() {/*
      ...
      var leaveTime = addDate(enterTime,1);*/
       ...
       var enterTime = formatDate(date);
      $('.entertime').text(enterTime); // 显示
      $('.leavetime').text(enterTime); // 显示
