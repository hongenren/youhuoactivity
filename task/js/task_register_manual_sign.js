function findByActivityCode(pageNumber,num){
    $(".warp_zy_h_List").empty();
    var arr={
        "searchValue": $(".memberSearchVal2").val(),
                ...
                if(data1!=null && data1!=""){
                    $.each(data1,function(index,val) {
                        ...
                        html+='<li>\n' +
                            ...
                            // '            <a data-json="'+JSON.stringify(postfile)+'" href="task_register_manual_sign1.html?type=1&subCode='+subCode+'&postfile='+encodeURI(JSON.stringify(postfile))+'">\n' +
                            // '                <span>'+orgName+'</span>\n' +
                    ...
                    if(data1.length>=0&&num==10){
                        $(".loadings").hide();
                        $(".lastext").show().text('数据有点多，请输入更详细的查询信息');
                    }
                }else{
                    $(".loadings").hide();
                    $(".lastext").show();
