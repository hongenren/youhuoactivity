var queryChangeApply=function(obj,type){
    var dataChangeJson=JSON.parse(decodeURI($(obj).attr('data-json')));
    var arr={
        "ticket":dataChangeJson.ticket,
        "type":type
    };
    $.ajax({
        xhrFields: {
        ...
        success:function(data){
            ...
            if(data.code==0){
                if(datas!==''){
                    if(type==10){
                        window.location.href='task_myOrder_transfer_progress.html?type='+datas.state+'&ticket='+dataChangeJson.ticket+'&getType='+type
                    }else if(type==20){
                        window.location.href='task_myOrder_out_progress.html?type='+datas.state+'&ticket='+dataChangeJson.ticket+'&getType='+type
                    }

                }else {
                    if(type==10){
                        window.location.href='task_myOrder_transfer.html?ticket='+dataChangeJson.ticket
                    }else if(type==20){
                        window.location.href='task_myOrder_out.html?ticket='+dataChangeJson.ticket
                    }
                }
            }else {
                layer.open({
...
var querySubmit=function(ticket){
    $.ajax({
        ...
        success:function(data){
            ...
            if(data.code==0){
                if(datas!==''){
                    window.location.href='task_myOrder_examine.html?ticket='+ticket
                }else {
                    window.location.href='task_myOrder_submit.html?ticket='+ticket
                }
            }else {
                layer.open({
                    content: '请求失败'
                                ...
                                if(subTaskList.length>0){
                                    $.each(subTaskList,function(index,item){
                                        ...
                                        if(type==20){
                                            //typeTxt='任务票';
                                            if(state==10){
                                                var timestamp = new Date(item.endTime).getTime();
                                                var nowdate = new Date().getTime(); // 获取当前时间
                                                if(timestamp>nowdate){
                                                    btns=
                                                        '<input onclick="querySubmit(\''+ticket+'\')"  type="button" value="任务提交" style="border: 1px solid red;color: red;">\n' +
                                                        '<input type="button" data-json="'+encodeURI(JSON.stringify(dataChangeJson))+'" value="转接" onclick="queryChangeApply(this,10)" style="border: 1px solid #72B81E;color: #72B81E;">\n' +
                                                        '<input type="button" data-json="'+encodeURI(JSON.stringify(dataChangeJson))+'" value="退出" onclick="queryChangeApply(this,20)" style="border: 1px solid deepskyblue;color: deepskyblue;">\n'
                                                }else{
                                                    btns=
                                                        '<input onclick="querySubmit(\''+ticket+'\')"  type="button" value="任务提交" style="border: 1px solid red;color: red;">\n'
                                                }
                                                ...
                                                btns=
                                                    '<input onclick="querySubmit(\''+ticket+'\')"  type="button" value="任务提交" style="border: 1px solid red;color: red;">\n' +
                                                    '<input type="button" value="评价" onclick="jump(\'task_order_evaluate.html?ticket='+ticket+'\')" style="border: 1px solid deepskyblue;color: deepskyblue;">\n'
                                            }
                                            ...
                                            btns=
                                                ...
                                                '<input onclick="jump(\'../post/send_notification.html?type=0&pkPostCode=' + item.subCode+'\');"  type="button" value="发通知" style="border: 1px solid red;color: red;">\n'
                                        }if(type==90){
                                            //typeTxt='子任务联系人票'
                                            btns=
