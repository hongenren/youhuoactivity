$(function () {
    ...
    function getList() {
        ...
        var uid = "userId:" + user_msg.userId;
        ...
        for (var i in endRes) {
            ...
            if (cookieNotread.length != 0) {
                ...
                var msg;
                if(endRes[i].msg.substring(0, 1) == '{'){
                    var msgObj = JSON.parse(endRes[i].msg);
                    if(msgObj.title == undefined || msgObj.title== ''){
                        msg = '[未知消息]';
                    }else{
                        msg = msgObj.title;
                    }
                }else{
                    msg = endRes[i].msg.substring(0, 5);
                }
                html += "</div>\n" +
                    "                    </div>\n" +
