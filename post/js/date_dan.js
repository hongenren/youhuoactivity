...
var date = function ($) {
  ...
  $.fn.hotelDate = function (options) {
    ...
    function initTemplate() {
      ...
      // 主容器模板
      var dateTemplate = '\n        <div class =\'date container c-gray\'>\n          <h4 class="tac bold">请选择活动开始\u548C<span class=\'c-red\'>结束</span>\u65F6\u95F4</h4>\n          <div class=\'close-btn\'>\u786E\u5B9A</div>\n        </div>      \n      ';
      ...
      $('.close-btn').on('click', function () {
        ...
        var leaveYear = $('.leave').parents('.day').siblings('.title').find('.y').text();
		 leaveYear.length === 1 ? leaveYear = '0' + leaveYear : false;
        ...
        if(enterTime == '--'){
            $('.entertime').text(leaveTime);
              $('.c-back').attr('style','display:none')
              $('.entertime').attr('style','display:none')
        }else{
              $('.entertime').text(enterTime); // 显示
              $('.c-back').attr('style','display:block')
              $('.entertime').attr('style','display:block')
        }
        ...
        $('.input-enter').val( $('.entertime').text());
        $('.input-leave').val($('.leavetime').text());
    ...
    function initDay() {
      ...
      leaveDay.length === 1 ? leaveDay = '0' + leaveDay : false;
      //var leaveTime = leaveYear + '-' + leaveMonth + '-' + leaveDay;
      var leaveTime = addDate(enterTime,1);
      //console.log(leaveTime)
      $('.entertime').text(enterTime); // 显示
      if(leaveYear==''||leaveMonth==''||leaveDay==''){
        $('.leavetime').text(enterTime);
      }else{
        $('.leavetime').text(leaveTime);
      }
	  
      $('.input-enter').val(enterTime);
      $('.input-leave').val(leaveTime);
    }
