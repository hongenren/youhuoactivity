    ...
    function initTemplate() {
      ...
      $('.action').each(function (index, item) {
        ...
        for (var i = 0; i < days + nowweek; i++) {
            var template = '';
            var enterYear = String(dateArr[index].getFullYear());
            enterYear.length === 1 ? enterYear = '0' + enterYear : false;
            var enterMonth = String((dateArr[index].getMonth() + 1));
            enterMonth.length === 1 ? enterMonth = '0' + enterMonth : false;
            var enterDay = String(i - nowweek + 1);
            enterDay.length === 1 ? enterDay = '0' + enterDay : false;
            var listDate = enterYear + '-' + enterMonth + '-' + enterDay;
          ...
          else if (input_enter.indexOf(listDate+",")!= -1) {
              // 默认入住时间
              listIndex++;
      ...
      $('.close-btn').on('click', function () {
          var jiekouTime = "", xianshiTime = "";
          $('.enter').each(function(index,domEle){
              ...
              enterDay.length === 1 ? enterDay = '0' + enterDay : false;
              jiekouTime  += enterYear + '-' + enterMonth + '-' + enterDay+",";
              xianshiTime  = enterYear + '-' + enterMonth + '-' + enterDay;
              if(index==0){
                  $('.input-enter').val(xianshiTime);
                  $('.entertime').html(xianshiTime);
              }
          });
          $('.input-enter').val(jiekouTime);
          $('.entertime').html($('.entertime').html()+"至"+xianshiTime);
          closeData();
      });
      var num = 0;
      // 时间选择
