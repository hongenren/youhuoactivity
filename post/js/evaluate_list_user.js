//管理员的评论 和自己的评论
    function get_evaluate_list(pageNumber, pageSize){
        ...
        $.ajax({
            ...
            type:"post",
            url:base_url + "html5/v1/station/postEvaluate/selectByPrjvUserIdAndType",
            data:json,
            headers : {'token':token},
            dataType:'json',
...
// 评价信息加载 html
        ...
        $.each(data,function(index,val){
            html += ' <li class="warp_gl_w_su">\
                        ...
                        if(name==null || name==undefined || name==""){
                            ...
                            html += '<span>'+name+'</span>';
                        if(type==20 && val.nickName!=""){
                            html += '<br /><span>对'+val.nickName+'的评价</span>';
                        }
                        html  +=         '</div>\
                        ...
                        if(val.imgs!=''){
                            ...
                            html +=     '</ul>\
                                ...
                                if(type==10) {
                                    html += '<span>我对【' + val.postName + '】评价</span>';
                                }else{
                                    html += '<span>来自【' + val.postName + '】评价</span>';
                                }
                            html +=     '</div>\
    ...
    $(function(){
            ...
            $.ajax({
                    ...
                    success:function(data){
                        if(data.code==0){
                            ...
                            setTimeout(function(){window.location.href ='./evaluate_list_user.html'},1000);
                        }else{
                            layer.open({
                                content: data.msg
                                ,skin: 'msg'
        ...
        $(".subhuifu").live("click",function(){
            ...
            var str = JSON.stringify({"postEvaluateId":id,"prjvUserId":user_msg.userId,"nickName":user_msg.userNickName,"headImg":user_msg.userHeadImg,"type":10,"content":content});
            $.ajax({
                xhrFields: {
                    withCredentials: true
                },
        ...
        $(".warp_gl_w_sl_2 a").live("click",function(){
            ...
            var data = {"id":id, "type":10, "prjvUserId": user_msg.userId};
            layer.open({
            content: '您确定要这么做么？'
                ,btn: ['确定', '取消']
                ,yes: function(index){    
                    $.ajax({
                        ...
                        type:"post",
                        url:base_url + "html5/v1/station/postEvaluate/delBySelf",
