    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code == 0) {
                ...
                $("#userpostName").val(detail.name);
                $("#userpostId").val(detail.userpostId);
                clickUserPost(detail.userpostId);

                $("#applyTimeType").val(detail.applyTimeType);
                $("#reqAttendTimes").val(detail.reqAttendTimes);
                $("#needNum").val(detail.needNum);
                $("#fieldAdmin").val(detail.fieldAdmin);
                $("#point").val(detail.point);
                $(".decrease").removeAttr('disabled');

                $("#serviceBeginTime").val(detail.serviceBeginTime);
                $("#serviceEndTime").val(detail.serviceEndTime);
                $("#applyBeginTime").val(detail.applyBeginTime);
                $("#applyEndTime").val(detail.applyEndTime);
                $("#welfare_name").html(detail.welfareName);
                $("#welfare").val(detail.welfare);
                welfare = $("#welfare").val();
                lat = detail.lat;
                lng = detail.lng;
                lat = detail.lat;
                var aca = $("#applyTimeType");
                aca.val(detail.applyTimeType);
                if(aca.val()=="") {
                    aca.css('color', '#9d9d9d');
                    $(".applyTime_li").hide();
                }else{
                    aca.css('color', '#000000');
                    $(".applyTime_li").hide();
                    if(aca.val()=="2"){
                        $(".applyTime_li").show();
                    }
                    ...
                    success: function (resp) {
                        if (resp.code == 0) {
                            ...
                            for(var z in Objects){
                                ...
                                if(welfare.indexOf(Objects[z].code)>-1){
                                    css = "selectedSpen";
                                }
                                var li='<li><span onclick="selectedSpan(this,\'welfare\',\''+Objects[z].name+'\',\''+Objects[z].code+'\')" class="'+css+'">'+Objects[z].name+'</span></li>';
...
function update_post(){
    ...
    if(checkParameters(data)) {
        $.ajax({
            ...
            type: 'post',
            url: base_url + "/html5/v1/station/updateInfo",
            ...
            success: function (data) {
                if (data.code == 0) {
                    ...
                    setTimeout(function () {jump("./post_management.html")}, 1000);
                } else {
                    layer.open({
                        content: data.msg
                        , skin: 'msg'
...
var ifPost = 0, oldId = "", serverTarget = "";
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code == 0) {
                ...
                queryOrgServiceObjects(serverTarget);
                province = detail.province;
                city = detail.city;
                district = detail.district;
                var questionList = "";
                $.each(detail.questionList, function (k, val) {
                    questionList += "<ul class=\"warp_hd_t_bu\" id=\"questionList\">\
...
function queryOrgServiceObjects(serverTargets){
    $.ajax({
