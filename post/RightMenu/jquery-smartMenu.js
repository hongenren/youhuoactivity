	...
	$.fn.smartMenu = function(data, options) {
							...
							if (text) {
								...
								if ($.isArray(obj.data) && obj.data.length) {
									htmlMenuLi = '<li class="'+ clKey +'li" data-hover="true">' + htmlCreateMenu(obj.data) +
										'<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'" style="color: white;"><i class="'+ clKey +'triangle"></i>'+ text +'</a>' +
									'</li>';
								} else {
									htmlMenuLi = '<li class="'+ clKey +'li">' +
										'<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'" style="color: white;">'+ text +'</a>' +
									'</li>';
								}
		...
		$(this).each(function() {
			this.onclick = function(e) {
				pkPostCode = $(this).attr("data-code");
				serviceBeginTime = $(this).attr("data-beginTime");
				serviceEndTime = $(this).attr("data-endTime");

				if($(this).attr('data-mainCode')){
            pkMainCode=$(this).attr('data-mainCode')
				}
				//回调
