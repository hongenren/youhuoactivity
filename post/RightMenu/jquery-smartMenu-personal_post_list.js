	...
	$.fn.smartMenu = function(data, options) {
		...
		$(this).each(function() {
			this.onclick = function(e) {
                applyCode = $(this).attr("data-code");
                var state = $(this).attr("data-state");
            	if($(this).html().indexOf("待审核")>-1){
                    return false;
                }
				//回调
				...
				//alert(jqueryMenu);smart_menu_a
                $(".smart_menu_a").each(function(){
                    if(state.indexOf("待打卡")>-1){
                    	if($(this).html()=="评价"){
                            $(this).parent().hide();
						}else{
                            $(this).parent().show();
                        }
                    }else if(state.indexOf("已退出")>-1){
                        jqueryMenu = null;
					}else{
                        if($(this).html()=="评价"){
                            $(this).parent().show();
